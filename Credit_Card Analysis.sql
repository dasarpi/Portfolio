...
VALUES
  ...
  SELECT COUNT(node_id) AS Number_of_nodes, region_id
  FROM customer_nodes
  GROUP BY region_id
  ORDER BY region_id;	
  ...
  SELECT COUNT(customer_id) AS Number_of_customer, region_id
  FROM customer_nodes
  GROUP BY region_id
  ORDER BY region_id;
 ...
 SELECT AVG(DATEDIFF(end_date, start_date)) AS Avg_days_reallocated
 FROM customer_nodes
...
SELECT 
    ...
    (SELECT reallocation_days
     FROM RankedReallocation r1
     WHERE r1.region_id = r.region_id
     AND r1.percentile_rank = 50
     LIMIT 1) AS median_days,
     
    -- 80th Percentile
    (SELECT reallocation_days
     FROM RankedReallocation r1
     WHERE r1.region_id = r.region_id
     AND r1.percentile_rank = 80
     LIMIT 1) AS percentile_80_days,
     
    -- 95th Percentile
    (SELECT reallocation_days
     FROM RankedReallocation r1
     WHERE r1.region_id = r.region_id
     AND r1.percentile_rank = 95
     LIMIT 1) AS percentile_95_days
FROM RankedReallocation r
GROUP BY region_id;

-- What is the unique count and total amount for each transaction type?

SELECT COUNT(customer_id) AS Unique_Count, SUM(txn_amount) AS total_amount, txn_type
FROM customer_transactions
GROUP BY txn_type
ORDER BY Unique_Count;

-- What is the average total historical deposit counts and amounts for all customers?

SELECT AVG(total_deposits) AS Avg_total_deposite,
AVG(total_deposit_amount) AS Avg_total_deposit_amt
FROM (
      SELECT customer_id, COUNT(*) AS total_deposits, SUM(txn_amount) AS total_deposit_amount
      FROM customer_transactions
...
SELECT YEAR(txn_date) AS txn_year, MONTH(txn_date) AS txn_month, COUNT(DISTINCT customer_id) AS customer_count
FROM customer_transactions
...
AND SUM(CASE WHEN txn_type IN ('purchase', 'withdrawal') THEN 1 ELSE 0 END) >=1;

-- What is the closing balance for each customer at the end of the month?

SELECT customer_id, YEAR(txn_date) AS txn_year, MONTH(txn_date) AS txn_month,
SUM(CASE WHEN txn_type = 'deposit' THEN 1 ELSE 0 END) > 1
AND SUM(CASE WHEN txn_type IN ('purchase', 'withdrawal') THEN 1 ELSE 0 END) AS closing_balance

FROM customer_transactions
WHERE txn_date <= last_day(txn_date) 
GROUP BY customer_id, YEAR(txn_date), MONTH(txn_date)
ORDER BY txn_year, txn_month, customer_id;
...
PreviousMonthBalances AS (
    SELECT 
        ...
        txn_month,
        closing_balance,
        LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY txn_year, txn_month) AS prev_month_balance
    FROM MonthlyBalances
)
SELECT 
    (COUNT(DISTINCT customer_id) * 100) / (SELECT COUNT(DISTINCT customer_id) FROM MonthlyBalances) AS percentage_customers_with_increase
FROM PreviousMonthBalances
WHERE 
    prev_month_balance IS NOT NULL AND 
    closing_balance > prev_month_balance * 1.05;
